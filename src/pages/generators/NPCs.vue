...
import { ref } from 'vue';
import { Settings2, UserCircle2Icon } from 'lucide-vue-next';
...
import { agesData } from '../../lib/selection_data/ages';
import { alignmentsData } from '../../lib/selection_data/alignments';
import { professionsData } from '../../lib/selection_data/professions';
...
const selectedProfession = ref({...professionsData[0]})
const selectedAge = ref({...agesData[0]})
const selectedAlignment = ref({...alignmentsData[0]})
...
async function generateQuest() {
  ...
  try{
    const response = await api.post('/npc/generate', {
      rpgSystem: selectedRpgSystem.value.value,
      npcLocation : selectedLocation.value.value,
      npcRace: selectedRace.value.value,
      npcProfession: selectedProfession.value.value,
      npcAge: selectedAge.value.value,
      npcAlignment: selectedAlignment.value.value
    })
...
function updateProfession(newValue: any) {
  selectedProfession.value = newValue
}

function updateAge(newValue: any) {
  selectedAge.value = newValue
}

function updateAlignment(newValue: any) {
  selectedAlignment.value = newValue
}
  ...
  <div id="quest-generator" class="flex flex-col max-h-full px-6 pb-6 pt-12 space-y-12">
    ...
    <div id="title" class="space-y-1">
      <div class="flex items-center space-x-3 text-slate-200">
        <UserCircle2Icon stroke-width="2" :size="40"/>
        <h2 class="text-heading-lg font-medium">NPC Generator</h2>
      </div>
      <span class="text-heading-xm font-normal text-slate-500">Breathe life into unique characters.</span>
    </div>
          ...
          <ul class="flex flex-col gap-4">
            ...
            <li>
              <SelectionInput
                ...
                :updateController="updateProfession"
              />
            </li>
            ...
            <li>
              <SelectionInput
                ...
                :data="alignmentsData"
                :updateController="updateAlignment"
              />
            </li>
